...
export function QueuePage() {
  ...
  const review = useMemo(() => {
    const byPhone = new Map(
      orders
        .filter(order => order.brand === "HOUSE190" && !order.eligible && order.phoneE164)
        .sort((a, b) => (a.purchaseAt?.getTime() ?? 0) - (b.purchaseAt?.getTime() ?? 0))
        .map(order => [order.phoneE164!, order]),
    );
  ...
  return <div className="mx-auto max-w-[1100px] p-5 md:p-8 xl:p-10">
    ...
    <section className="mt-5 rounded-[22px] border border-[#e5ae25]/40 bg-white p-5">
      ...
      <div className="mt-4 space-y-3">{review.map(order => <article key={order.id} className="rounded-2xl border border-[#e5ae25]/30 bg-[#fffdf6] p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h3 className="text-sm font-semibold">{order.customerName}</h3><Badge tone="green">HOUSE190</Badge><Badge tone="amber">Bloqueado</Badge></div><button onClick={() => copyPhone(order.id, order.phoneE164!)} className="mt-2 inline-flex items-center gap-1.5 font-mono text-xs text-[#765506] hover:underline">+{order.phoneE164}{copied === order.id ? <Check className="size-3.5"/> : <Copy className="size-3.5"/>}</button><p className="mt-3 flex items-center gap-1.5 text-xs text-black/50"><ShieldAlert className="size-3.5"/>{reasonLabel(order.reason)}</p></div><span className="text-[10px] text-black/35">{order.purchaseAt?.toLocaleDateString("pt-BR")}</span></div></article>)}{!loading && !review.length && <Empty title="Nenhum pedido House 190 aguardando revisão." text="Confira se os produtos foram classificados e reprocessados."/>}</div>
    </section>
